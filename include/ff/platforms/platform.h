...
#define NOFF_INLINE __declspec(noinline)
...
FF_INLINE void WMB() {}
FF_INLINE void PAUSE() {}

#include <BaseTsd.h>
typedef SSIZE_T ssize_t;

FF_INLINE static int posix_memalign(void **memptr,size_t alignment, size_t sz)
{
...
FF_INLINE static void posix_memalign_free(void* mem)
{
    _aligned_free(mem);
}
...
FF_INLINE static int usleep(unsigned long microsecs) {
  if (microsecs > 100000)
...
FF_INLINE static int gettimeofday(struct timeval *tv, struct timezone *tz)
{
  FILETIME ft;
